...
def answer_hall(hall_name):
    ...
    else if 'meal' not in session.attributes.keys():
        ask_msg = render_template('inter-ask-meal')
    else if 'date' not in session.attributes.keys():
...
def answer_meal(meal):
    ...
    meal = request.intent.slots.hall.resolutions.resolutionsPerAuthority[0]['values'][0]['value']['id']
...
def answer_date(date):
    ...
    date = request.intent.slots.hall.resolutions.resolutionsPerAuthority[0]['values'][0]['value']['id']
